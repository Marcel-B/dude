...
public class AuthorizeModel : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        if (request.HasPrompt(Prompts.Login))
        {
            ...
            var prompt = string.Join(" ", request
                .GetPrompts()
                .Remove(Prompts.Login));

            var parameters = Request.HasFormContentType
                ? Request
                    .Form.Where(parameter => parameter.Key != Parameters.Prompt)
                    .ToList()
                : Request
                    .Query.Where(parameter => parameter.Key != Parameters.Prompt)
                    .ToList();
        ...
        if (request.HasAcrValue("1")) identity.AddClaim(new Claim(Claims.AuthenticationContextReference, "1"));
