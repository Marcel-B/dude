...
public class MqttWorker : BackgroundService
{
  ...
  public MqttWorker(ILogger<MqttWorker> logger, IServiceProvider serviceProvider)
  {
    _logger = logger;
  ...
  protected override async Task ExecuteAsync(CancellationToken stoppingToken)
  {
    ...
    do
    {
      try
      {
        ...
        var application = configuration.GetSection("Application").Get<Application>();
        _subscriptions = application?.Subscriptions ?? throw new InvalidOperationException("No subscriptions found.");
        ...
        var clientOptions = new MqttClientOptions()
        {
          ...
          Credentials = new MqttClientCredentials(application.UserName, Encoding.ASCII.GetBytes(application.Password)),
